...
# path = "./data_key12_3_7_recover/"
path = "./key_recovery_attack/key_data_3_7/"
wrongKey = './key_recovery_attack/mean_std_rscp/'
model = './key_recovery_attack/saved_model/rscp/'


net6 = load_model(model+"6_1_10_rscp_distinguisher.h5",compile=False)
net7 = load_model(model+"7_1_10_rscp_distinguisher.h5",compile=False)

m7 = np.load(wrongKey+"data_wrong_key_mean_7r_1g_rscp.npy");
s7 = np.load(wrongKey+"data_wrong_key_std_7r_1g_rscp.npy"); s7 = 1.0/s7

m6 = np.load(wrongKey + "data_wrong_key_mean_6r_1g_rscp.npy");
s6 = np.load(wrongKey + "data_wrong_key_std_6r_1g_rscp.npy"); s6 = 1.0/s6
...
def run(n, nr=12, num_structures=10000000, it=500, cutoff1=6, cutoff2=9, neutral_bits=[[20],[22],[13],[12,19],[14,21],[6,29]], keyschedule='real',net=net7, net_help=net6, m_main=m7, s_main=s7,  m_help=m6, s_help=s6, verify_breadth=None):
  print("Checking Speck32/64 implementation.")
  if (not sp.check_testvector()):
    print("Error. Aborting.")
    return(0)
  arr1 = np.zeros(10, dtype=np.uint16); arr2 = np.zeros(10, dtype=np.uint16)
  # 返回自epoch以来的当前时间(以秒为单位)，如果系统时钟提供，可能会出现一秒的分数
  t0 = time()
  data = 0; av=0.0; good = np.zeros(10, dtype=np.uint8)
  ...
  for i in range(n):
    ...
    # 将结果转换为列表结构 {list: 8}
    new_ct = repeated_list[:, :, :350]
    g = find_good(new_ct, key); g = np.max(g); good[i-n] = g
    ...
    # 异或运算
    arr1[i-n] = guess[0] ^ key[nr-1]; arr2[i-n] = guess[1] ^ key[nr-2]
    print("Difference between real key and key guess: ", hex(arr1[i-n]), hex(arr2[i-n]))
  # 返回自epoch以来的当前时间(以秒为单位)，如果系统时钟提供，可能会出现一秒的分数
  t1 = time()
  print("Done.")
...
folder="./key_recovery_attack/3_7/"
# 写文件 'wb'：以二进制写模式打开文件
np.save(folder + 'run_sols1_'+'12_3_7_ncround.npy', arr1)
np.save(folder + 'run_sols2_'+'12_3_7_ncround.npy', arr2)
np.save(folder + 'run_good_'+'12_3_7_ncround.npy', good)
np.save(open('run_sols1_'+'3_7_nc.npy','wb'),arr1)
np.save(open('run_sols2_'+'3_7_nc.npy','wb'),arr2)
np.save(open('run_good_'+'3_7_nc.npy','wb'),good)
